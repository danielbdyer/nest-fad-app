...
import { ServiceLineWhereInput } from "../../serviceLine/base/ServiceLineWhereInput";
...
export class LocationControllerBase {
  ...
  async findManyServiceLine(
    @common.Req() request: Request,
    @common.Param() params: LocationWhereUniqueInput,
    @nestAccessControl.UserRoles() userRoles: string[]
  ): Promise<ServiceLine[]> {
    const query: ServiceLineWhereInput = request.query;
    const permission = this.rolesBuilder.permission({
      role: userRoles,
      action: "read",
      possession: "any",
    ...
    const results = await this.service.findServiceLine(params.id, {
      where: query,
      select: {
        createdAt: true,
        id: true,
